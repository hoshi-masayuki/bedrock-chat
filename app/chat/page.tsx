...
export default function Chatbot() {
  ...
  const onSend = async (e: any) => {
    ...
    if (sendCount >= 0 && sendCount <= 4) {
      ...
      _prompt += chats.filter(chat => chat.type === "user").map(chat => chat.text).join(" ") + `これまでのユーザーの返答をすべて考慮して以下のフォーマットに必要な情報がそろっていると判断した場合、「質問は以上になりますお時間いただきありがとうございました。」とそのままお伝えしインタビューを終了してくださいインタビューを終了してください。不足している情報がある場合は、追加で質問してください。
      インタビュー結果で埋めるべき項目
